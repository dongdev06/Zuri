...
try {
    ...
    else logger.loader("Không thể tải được config.json", "error");
}

try {
    for (const key in configValue) global.config[key] = configValue[key];
    logger.loader("Tiến hành tải config");
}
catch { logger.loader("Không thể tải được config", "error") }
    ...
    if (process.env.KEY && global.config.encryptSt) {
      ...
      writeFileSync(appStateFile, loginState)
        ...
        global.client.api = loginApiData
          logger(`[ DATA ZURI] -> Đang tiến hành kết nối với máy chủ của zuri project`);
     logger.loader(`[ DATA ZURI ] -> Kết nối thành công đến máy chủ của zuri project`)
